...
import com.drops.controller.MenuController;
import com.drops.tools.SpringContextUtil;
...
public class ResouceDowngradeAspect {
    ...
    public Logger log = Logger.getLogger(ResouceDowngradeAspect.class);
    ...
    private RateLimiter resetRateLimiterIfMaxThresholdChange(String signature, int maxThreshold) {
        ...
        if(orginMaxThresholdMap.containsKey(signature)){
            ...
            if(orginMaxThreshold != maxThreshold){
                log.info("signature:{"+signature+"}-->orginMaxThreshold is :{"+orginMaxThreshold+"},currentMaxThreshold is :{"+maxThreshold+"},rateLimiter maxThreshold will change to -->{"+maxThreshold+"}");
    ...
    private Object downgrade(ProceedingJoinPoint pjp, ResouceDowngrade resouceDowngrade)throws Throwable{
        ...
        if(counter.get(signature).get() > maxThreshold){
            log.info("signature:{"+signature+"},current requestCount:{"+counter.get(signature).get()+"},maxThreshold:{"+maxThreshold+"}");
            return this.fallback(pjp, resouceDowngrade);
        }
        Object result = null;
        try {
